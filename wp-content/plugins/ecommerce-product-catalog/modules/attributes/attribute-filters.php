...
class ic_attribute_default_filters {
	...
	function apply_size_filter( $query ) {
		...
		foreach ( $size_fields as $field_name => $label ) {
			if ( is_product_filter_active( $field_name ) ) {
				...
				if ( $min_max[ 0 ] === $this->get_min( $field_name ) || $min_max[ 1 ] === $this->get_max( $field_name ) ) {
					continue;
				}
	...
	function get_size_filters() {
		...
		if ( empty( $filter_fields ) ) {
			$filter_fields = __( 'One size available.', 'ecommerce-product-catalog' );
		}
	...
	function get_min( $field_name ) {
		$values	 = array_unique( $this->get_meta_values( $field_name ) );
		$values	 = array_merge( $values, $this->get_meta_values( '_1' . $field_name . '_filterable' ) );
		natsort( $values );
	...
	function get_meta_values( $key = '', $type = 'al_product', $status = 'publish' ) {

		global $wpdb, $ic_product_filters_query;
		if ( empty( $key ) ) {
			return;
		}
		...
		if ( is_ic_taxonomy_page() || !empty( $ic_product_filters_query ) ) {
