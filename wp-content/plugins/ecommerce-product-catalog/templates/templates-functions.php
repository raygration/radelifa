...
function ic_is_woo_template_available() {
	if ( !is_integration_mode_selected() && !is_ic_shortcode_integration() && !isset( $_GET[ 'test_advanced' ] ) ) {
...
function ic_try_woo_template_file() {
	if ( !is_integration_mode_selected() ) {
		...
		if ( get_option( 'ic_block_woo_template_file', 0 ) === $theme ) {
			return false;
		}
			...
			if ( !file_exists( $product_adder_path ) ) {
				...
				if ( ic_string_contains( $main_file_contents, 'woocommerce_content' ) && copy( $path, $product_adder_path ) ) {
					...
					if ( ic_string_contains( $file_contents, 'woocommerce_content' ) ) {
						...
						ic_handle_woo_template_error( $product_adder_path );
						return true;
					}
				}
			} else {
...
function ic_handle_woo_template_error( $product_adder_path ) {
	$theme = get_option( 'template' );
	if ( get_option( 'ic_allow_woo_template_file', 0 ) === $theme ) {
		return true;
	}
	if ( ic_check_for_errors( $product_adder_path ) ) {
		unlink( $product_adder_path );
		update_option( 'ic_block_woo_template_file', $theme );
	} else {
		update_option( 'ic_allow_woo_template_file', $theme );
	}
	$url = 'http';
	if ( is_ssl() ) {
		$url .= 's';
...
function ic_check_for_errors( $path ) {
